(function(){
  ...
  function applyFilter(group){
    ...
    cards.forEach(card=>{const show=group==='all'||card.dataset.group===group;card.hidden=!show;if(show)visible+=1});
    count.textContent=String(visible).padStart(2,'0');
    const url=new URL(location.href);group==='all'?url.searchParams.delete('set'):url.searchParams.set('set',group);history.replaceState({},'',url);
  }
  ...
  cards.forEach(card=>card.addEventListener('click',()=>{image.src=card.dataset.file;image.alt=card.querySelector('img').alt;title.textContent=card.dataset.title;viewer.classList.add('is-open');viewer.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';close.focus()}));
