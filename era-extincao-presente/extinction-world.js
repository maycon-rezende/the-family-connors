(()=>{
  ...
  const setCinemaBeat=index=>{
    ...
    if(index===2){
      ...
      cinemaTimers.push(setTimeout(()=>awakening.classList.remove('is-flashing','is-shaking'),900));
    }
    const status=['AGUARDANDO CONEXÃO','ARQUIVO RECUPERADO','IMPACTO DETECTADO','TEMPO CORROMPIDO','SINAL ESTABILIZADO'];
  ...
  const startCinema=()=>{
    ...
    [[1,250],[2,3550],[3,6850],[4,10150]].forEach(([beat,delay])=>cinemaTimers.push(setTimeout(()=>setCinemaBeat(beat),delay)));
  ...
  const enterWorld=()=>{
    ...
    try{sessionStorage.setItem('extinction-cinema-v2','1')}catch(error){}
    if(introAudio){
      const fade=setInterval(()=>{
        introAudio.volume=Math.max(0,introAudio.volume-.04);
        if(introAudio.volume<=.01){clearInterval(fade);introAudio.pause()}
      },70);
    }
  };
  ...
  try{alreadyAwake=sessionStorage.getItem('extinction-cinema-v2')==='1'}catch(error){}
  ...
  if(cursor&&matchMedia('(pointer:fine)').matches){
    ...
    document.querySelectorAll('a,button').forEach(element=>{
      element.addEventListener('mouseenter',()=>cursor.querySelector('span').textContent='ACESSAR');
      element.addEventListener('mouseleave',()=>cursor.querySelector('span').textContent='RASTREAR');
    });
  }
  ...
  document.querySelectorAll('.reveal').forEach(element=>reduced?element.classList.add('is-visible'):observer.observe(element));

  document.querySelectorAll('.survivor').forEach(button=>button.addEventListener('click',()=>{
  ...
  document.querySelectorAll('[data-open-past],a[href="arquivo-passado.html"]').forEach(link=>link.addEventListener('click',event=>{
    if(reduced)return;
