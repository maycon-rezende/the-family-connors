(() => {
  ...
  function dust(x, y) {
    ...
    mote.style.setProperty('--alpha', 0.28 + Math.random() * 0.42);
  ...
  document.addEventListener('pointerover', (event) => {
    const interactive = event.target.closest(
      'a,button,input,textarea,select,[role="button"],.member-card,.memory-card,.lineage-person,.gallery-item,.memory'
    );
